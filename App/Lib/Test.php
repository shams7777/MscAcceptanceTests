...
class Test
{
    ...
    {
        //$arr = explode(')',trim($this->getTitle()));
        $arr = substr($this->getTitle(), -1);
        $usa = substr($this->getTitle(), 0,3);
      if ($usa == 'USA')
            {
            $this->description = 'Shipping costs for The rest of the world depending on Products in Cart and Customer Group';
            }
       elseif($arr[0]=='1') {
            $this->description = 'Shipping costs for Germany depending on Products in Cart and Customer Group ';

        }
        elseif ( $arr[0]=='2')
        {
            $this->description = 'Shipping costs for Europe depending on Products in Cart and Customer Group';
        }
        elseif ($arr[0]=='3' || $arr[0]=='4')
        {
            $this->description = 'Availability of Payment Options depending on products in cart and customer properties';
        }
        elseif ($arr[0]=='5')
        {
            $this->description = 'Display of partial delivery option depending on customer group and product availability';
        }
        elseif ($arr[0]=='6')
        {
            $this->description = 'Display of Price and VAT details depending on customer group (brutto/netto)';
        }
        elseif ($arr[0]=='7')
        {
            $this->description = 'Availability of Express Shipping Option depending on Product and Customer Group';
        }
        elseif ($arr[0]=='8')
        {
            $this->description = 'Availability of Checkout Option depending on  Customer Group';
        }
